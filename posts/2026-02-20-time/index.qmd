...
Wait a second...is that integral a right hairy bastard?  I was working with ChatGPT to relearn some more about numerical integration, I asked which numerical integration method would be best for this integrand (as if I wasn't going to just use the trapezoid rule with a fine grid and a large limits of integration).  ChatGPT brought be down the following line of argument (which I have written in my own words).

A property of Gaussians allow us to re-write the integral as
...
\int_{-\infty}^{\infty}
\lambda \,
\Phi\left(\dfrac{\lambda - S(N)}{s_\lambda} \right)
f(\lambda)\, d\lambda.
$$

from which we can identify the integral as a legitimate expectation, namely
...
Introduce an auxiliary random variable. Let
...
independent of $\lambda$. Then

$$
\Phi\!\left(\dfrac{\lambda - S(N)}{s_\lambda}\right)
=
\Pr(Y \le \lambda \mid \lambda) = E[\mathbf{1}(Y\leq \lambda \mid \lambda)]
$$

Here, I have used the fact that a probability can be expressed as the expectation of some indicator variable. This is handy because the expectation becomes

$$
=
\mathbb{E}\!\left[
\lambda \,
\mathbb{E}[\mathbf{1}(Y \le \lambda)\mid \lambda]
\right] = E\!\left[\lambda \mathbf{1}(Y \le \lambda)\right].
$$

via the law od total expectation. That’s just a truncated bivariate normal moment, which I guess has closed form!  

Since $Y \perp \lambda$ then

$$
\lambda - Y
\sim
\operatorname{Normal}(\mu - S(N),\; \tau^2 + s_\lambda^2).
$$

and so the expectation can be written as 
...
\mu \, \Phi(a)
+
\dfrac{\tau^2}{\omega} \, \phi(a) \>.
$$

Here, $\omega^2 = \tau^2 + s_\lambda^2$, $a = \dfrac{\mu - S(N)}{\omega}$, and $\phi$ is the standard normal density.


A few things are worth noting.  First, the dependence on $N$ enters entirely through $s_\lambda$, which affects both $\omega$ and $a$. As $N \to \infty$, $s_\lambda \to 0$, and we recover the Clairvoyant distribution:
...
since $S(N) \rightarrow 0$ as $N$ becomes large.


Finally, this formula works for all three decision rules (see below!). The only thing that changes across rules is the critical threshold $S(N)$. Once you have that, everything collapses to the same expression.


```{r closedform-comparison}
#| fig-cap: "Closed-form expected impact curves for all three decision rules: probability to beat control ($c = 0.95$, solid), 95% credible interval excluding zero (dashed), and M > 0 (dotted). The red horizontal line is the Clairvoyant distribution expectation — the upper bound achieved with perfect information."
lns <- seq(2, 5, 0.25)
ns  <- 10^lns
...
# Clairvoyant limit: se_lift -> 0, threshold -> 0, so E[lambda * 1(lambda > 0)]
clairvoyant <- closedform_impact_from_crit(lambda_crit = 0, se_lift = 0)

ylim <- range(0, closed_prob, closed_ci, closed_M, clairvoyant)

plot(lns, closed_prob, type = 'l', lwd = 2, lty = 1,
     xlab = expression(log[10](N)), ylab = 'Expected Impact', yaxt = 'n', ylim = ylim)
axis(2, at = axTicks(2), labels = scales::percent(axTicks(2)))
grid()
lines(lns, closed_ci, lwd = 2, lty = 2)
lines(lns, closed_M,  lwd = 2, lty = 3)
abline(h = clairvoyant, col = 'red', lwd = 2)
legend('bottomright',
       legend = c(
         expression(P(lambda > 0 ~ "|" ~ hat(lambda)) > 0.95),
         expression("95% CI for " * lambda > 0),
         expression("M > 0"),
         "Clairvoyant"
       ),
