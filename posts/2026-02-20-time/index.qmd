...
Define $\omega^2 = \tau^2 + s_\lambda^2$ as the total variance of the observed lift. Using the properties of the multivariate normal distribution, the expected value of $\lambda$ over the truncation $\hat \lambda > S(N)$ is:

$$E[\lambda \cdot \mathbf{1}(\hat \lambda > S(N))] = \mu \Phi(a) + \frac{\tau^2}{\omega} \phi(a)$$

where the standardized distance $a$ is:

$$a = \frac{\mu - S(N)}{\omega}$$

Substituting $S(N)$ back into our expression for $a$ gives a fully analytic expression for expected impact as a function of sample size, valid for any prior mean $\mu$ and prior variance $\tau^2$:
