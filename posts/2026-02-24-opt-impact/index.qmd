...
#| fig-cap: "Simulated quarterly impact distributions (shaded bands: 5th–95th percentile; open markers: mean and median) overlaid on the closed-form curves (lines) from the previous plot. Simulation uses 10,000 quarters per sample size on a coarser grid. At large $N$, fewer than one experiment fits in a quarter, so impact collapses to zero."
...
sim_median_prob <- sapply(sims, \(s) median(s$prob))
sim_median_ci   <- sapply(sims, \(s) median(s$ci))
sim_lo_prob     <- sapply(sims, \(s) quantile(s$prob, 0.05))
sim_hi_prob     <- sapply(sims, \(s) quantile(s$prob, 0.95))
sim_lo_ci       <- sapply(sims, \(s) quantile(s$ci,   0.05))
sim_hi_ci       <- sapply(sims, \(s) quantile(s$ci,   0.95))
...
layout(matrix(c(1, 2, 3, 3), nrow = 2, byrow = TRUE), heights = c(10, 1.5))
for (r in rules) {
  ...
  points(lns_sim, r$sim_median, pch = 2, col = 'black')
}
par(mar = c(0, 0, 0, 0))
plot.new()
legend('center', horiz = TRUE,
       legend = c("Closed-form", "Sim mean", "Sim median", "5th–95th pct."),
       lty    = c(1,  NA, NA,  NA),
       pch    = c(NA,  1,  2,  15),
       col    = c('black', 'black', 'black', adjustcolor('black', alpha.f = 0.12)),
       lwd    = 2, cex = 0.8, pt.cex = c(NA, 1, 1, 2))
layout(1)
```
