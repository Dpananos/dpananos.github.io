...
>If no informaiton -- other than the data $y$ -- is available to distinguish any of the $\theta_j$'s from any of the others and no ordering or grouping of the parameters can be made, one must assyme symmetry among the parameters in ther prior distribution.

The symmetry point is, in a way, reffering to the definition of exchangebility in terms of permutations (group theorists will know). As such, a useful gut check (as we will see later in this post) is to ask ones self if experiment results can be grouped in some, even if loose, sense.  If they can, exchangeability may not be  justifiable -- but more on that later.
...
Experiments generally contain more than one metric. There may be a __primary metric__ which the treatment is explicitly intended to move, several __secondary metrics__ which help us understand how the treatment works, and __guardrail metrics__ which we monitor largely to make sure that the treatment does not cause harm elsewhere.
...
The claim is instead that, *a priori*, we would not be indifferent to the assignment. Handed a large revenue effect, we would sooner attach it to the checkout experiment than to the latency experiment, and that preference is exactly what undermine a claim of exchangeability. The role revenue plays in an experiment is information the experiment label is carrying about $\theta_k$. In the words of Gelman and co-aithors, there is information availabl with which to distinguish some of the treatment effecs from others; a rough grouping of parameters could be made.

This gives us a relatively simple restriction when constructing an exchangeable set of experiments: if we are trying to learn the distribution of treatment effects on some metric, we should probably focus primarily on experiments for which that metric was itself a __primary metric__ (you could do also focus secondary metrics or guardrails, but I imagine focusing on primary metrics might be more useful).
...
Once again, then, experiment identity makes certain permutations less plausible than others -- that is, we can use existing information to group effects. If we know that one experiment directly changes the checkout flow and another makes a relatively small change to the home page, we would not be indifferent to swapping their revenue effects.

Perhaps the more general point is that sharing a metric is not enough. We should also ask whether that metric occupies approximately the same __role__ in each experiment, and whether the treatments have a comparable opportunity to affect it.
...
One response would be to suppose that experiments are exchangeable __conditional on team__. For example, we might imagine something like

$$
\theta_k 
\sim
\mbox{Normal}(\mu_k, \tau_k^2).
$$
...
Before constructing any exchangeable set of experiments, there is one class of experiments which should simply be removed: __A/A tests__.
